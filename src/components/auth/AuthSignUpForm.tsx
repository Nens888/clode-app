...
export function AuthSignUpForm() {
  ...
  return (
    <div className="py-10">
      <AuthCard
        title="Регистрация"
        subtitle="Мы отправим код подтверждения на почту"
      >
        <form
          className="space-y-4"
          onSubmit={async (e) => {
        ...
        >
          ...
          >
            {sent ? "Код отправлен" : loading ? "Отправляем..." : "Получить код"}
          </button>

          <div className="text-center text-sm text-white/45">
            Уже есть аккаунт?{" "}
            <Link href="/auth/sign-in" className="text-white/80 hover:text-white">
              Войти
            </Link>
          </div>
        </form>
